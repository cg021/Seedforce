...
function resolveModuleFromAlias(specifier, moduleRecord, opts) {
    ...
    return utils_1.createRegistryEntry(entry, specifier, opts);
}
function resolveModuleFromDir(specifier, moduleRecord, opts) {
    ...
    const entry = utils_1.getModuleEntry(moduleDir, name, opts);
    return utils_1.createRegistryEntry(entry, specifier, opts);
}
function resolveModuleFromNpm(specifier, npmModuleRecord, opts) {
    ...
    const lwcConfig = utils_1.getLwcConfig(packageDir);
    utils_1.validateNpmConfig(lwcConfig, { rootDir: packageDir });
    let exposedModules = lwcConfig.expose;
    let reverseMapping;
    if (aliasMapping) {
        utils_1.validateNpmAlias(lwcConfig.expose, aliasMapping, { rootDir: packageDir });
        exposedModules = utils_1.remapList(lwcConfig.expose, aliasMapping);
        reverseMapping = utils_1.transposeObject(aliasMapping);
    }
...
function resolveModuleRecordType(specifier, moduleRecord, opts) {
    const { rootDir } = opts;
    if (utils_1.isAliasModuleRecord(moduleRecord)) {
        return resolveModuleFromAlias(specifier, moduleRecord, { rootDir });
    }
    else if (utils_1.isDirModuleRecord(moduleRecord)) {
        return resolveModuleFromDir(specifier, moduleRecord, { rootDir });
    }
    else if (utils_1.isNpmModuleRecord(moduleRecord)) {
...
function resolveModule(importee, dirname, config) {
    ...
    const rootDir = utils_1.findFirstUpwardConfigPath(path_1.default.resolve(dirname));
    const lwcConfig = utils_1.getLwcConfig(rootDir);
    let modules = lwcConfig.modules || [];
    if (config) {
        const userConfig = utils_1.normalizeConfig(config, rootDir);
        modules = utils_1.mergeModules(userConfig.modules, modules);
    }
